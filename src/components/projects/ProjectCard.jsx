...
const ProjectCard = ({
  ...
  return (
    ...
    ${
      reverse
        ? "[transform:perspective(800px)_rotateY(-25deg)_scale(0.8)_rotateX(10deg)] hover:[transform:perspective(800px)_rotateY(15deg)_translateY(50px)_rotateX(10deg)_scale(0.95)]"
        : "[transform:perspective(800px)_rotateY(25deg)_scale(0.8)_rotateX(10deg)] hover:[transform:perspective(800px)_rotateY(-15deg)_translateY(-50px)_rotateX(10deg)_scale(0.95)]"
    }
    hover:shadow-xl`}
        />
      ...
      <div className="flex flex-col justify-center items-center text-white p-4 w-full lg:w-2/5">
        <h4 className="font-black font-cyborg mb-2 px-4 py-2 flex items-center text-lg">
          {title}
          <a href={link} target="_blank" rel="noreferrer" className="z-30">
